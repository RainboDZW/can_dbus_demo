...
/* 移除旧的单缓冲区变量 */
// static uint8_t dbus_rx_buffer[SBUS_RX_BUF_NUM];    /* 移除单缓冲区 */
...
/**
 * @brief remote control usart RxDMA MultiBuffer - 双缓冲区系统
 */
uint8_t SBUS_MultiRx_Buf[2][SBUS_RX_BUF_NUM] = {0};
...
void Remote_Message_Moniter(Remote_Info_Typedef  *remote_ctrl)
{
  ...
  {
    /* 只清空通道数据，不清空整个结构体 */
    memset(remote_ctrl->rc.ch, 0, sizeof(remote_ctrl->rc.ch));
    memset(remote_ctrl->rc.s, 0, sizeof(remote_ctrl->rc.s));
    memset(&remote_ctrl->mouse, 0, sizeof(remote_ctrl->mouse));
    memset(&remote_ctrl->key, 0, sizeof(remote_ctrl->key));
...
void DBUS_Init(void)
{
    ...
    /* 清空双缓���区 - 移到后面，确保系统稳定后再操作 */
    // memset(SBUS_MultiRx_Buf, 0, sizeof(SBUS_MultiRx_Buf));
...
/**
  * @brief  移除旧的空闲中断回调函数，现在使用HAL_UARTEx_RxEventCallback
  */
void DBUS_UART_IDLECallback(void)
{
    /* 此函数已被HAL_UARTEx_RxEventCallback替代 */
    /* 新的双缓冲区DMA系统会自动处理���收 */
}
